...
:warning: なお、ネームサーバの設定が完了しても、反映されるまでには時間がかかります。反映されるまでは、登録したドメインにアクセスしてもレジストラの広告ページが表示されるので注意してください。

ネームサーバが反映されたかどうかは`dig`コマンドで確認することができます。
...
上記のようにCloudFlareのネームサーバが表示されていればDNSに反映されたことを意味します。反映されていない場合はお待ちください。ちなみにDNSの反映は48時間くらいかかることもあるそうです。ぼくの場合は夜中に設定して、昼間に反映されました。
...
これで`http://`でアクセスしても`https://`に自動的にリダイレクトされるようになります。なお、ネームサーバの反映が完了していないと `Always Use HTTPS` が表示されない場合があります。その場合はネームサーバの反映が完了するまでお待ち下さい。
...
# さいごに
ネームサーバの反映が終了していて、設定が間違っていなければ、SSL+独自ドメインで GitHub Pages に公開されているはずです。お疲れさまでした！

...
Windows 10 は仕様上、4 GB 以上のメモリを使用するデバイスが接続された際にリソース不足というエラーを出します。いわゆるエラー 12 と呼ばれるものです。
...
そこには、`Boot Camp` (Windows 10) と `Macintosh HD` (macOS) の他に、`EFI Boot` という起動ディスクがあると思いますので、それを選択して起動します。

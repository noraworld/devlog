...
iPhone の設定アプリから Bluetooth の項目に行き、iPhone 側から Raspberry Pi に接続することはできるのだが、Raspberry Pi 側から iPhone に接続しようとすると失敗してしまう。

それもたまに失敗するとかではなく 100 % 失敗するので、何か原因があるのだろう。
...
設定でマイクに関しては認識させないということができるはずなので、設定方法がわかったら追記する。

...
| プロキシサーバで特定のドメインの拒否設定を行う | • 各種デバイスに明示的にプロキシサーバを指定しなければならない<br>• 簡単に設定を外せてしまう<br>• プロキシサーバを経由しないアプリケーションでは無効 |
...
今まで試していたのは DNS やプロキシなど、上のレイヤーからのアプローチだったのですが、やはり TCP/IP レイヤーなどでないと完全に制御するのは難しいなということがわかりました。

しかし、実際にルータでパケットフィルタリングしてみると、性能面での問題や表示速度の問題が浮き彫りになりました。やはりパフォーマンスを犠牲にせずに TCP/IP 層で制限をかけるのは難しいなという結論に至りました。
...
通常の家庭では、ネットワークはプロバイダが提供するルータあるいは市販で購入したルータを基軸に、単一のネットワークが形成され、そのネットワーク内に各々のデバイスが属していると思います。

今回はネットワークを 2 つのセグメントに分離し、片方は本物のルータ (プロバイダのルータや市販のルータ等) と Raspberry Pi のみが属するネットワーク、もう片方は Raspberry Pi と各種デバイスが属するネットワークというように分けます。
...
ネットワーク A は `192.168.3.0/24` で、ネットワーク B は `192.168.82.0/24`[^2] としています。また Raspberry Pi の IP アドレスはそれぞれ `192.168.3.2` と `192.168.82.2` となっています。この IP アドレスは他の端末と重複しないように注意してください。

上位 DNS サーバは何でも良いのですが筆者は `1.1.1.1` (Cloudflare DNS) を使用しています。

なお今後の説明ではネットワーク A は `192.168.3.0/24` で、ネットワーク B は `192.168.82.0/24` として説明します。別の範囲を設定した場合は適宜読み替えてください。

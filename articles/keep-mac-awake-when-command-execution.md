...
`exit` コマンドがこのリストの中に入っているので、インタラクティブシェルで `exit` コマンドを実行したときは `caffeinate` コマンドは実行されないというわけです。
...
ということは、`precmd()` が実行されて `caffeinate` コマンドが実行されるのは良いものの、`precmd()` が実行されないので `caffeinate` コマンドのプロセスが残り続けてしまいます。こうすると不眠不休の Mac になってしまいます。

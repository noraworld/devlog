...
## Zenn における記事のルールについて
* Qiita と違って HTML は解釈されない
  * リンクを新しいタブで開かせたり、画像のサイズを設定したり、文字に着色したりするために HTML を使用することができない
* topics に記号が入っていると `zenn preview` でエラーが出るが、一部の記号は無視しても問題なさそう
  * `Node.js` という topics には `.` が含まれていてプレビューではエラーになったが、実際に公開してみたら問題なかった
* 記事のタイトルが 60 文字を超えると `zenn preview` でエラーが出るが、実際には 70 文字まで
  * 60 文字を超えてもプレビューでエラーが出るだけで、デプロイは成功する
  * しかし 70 文字を超えるとデプロイに失敗し記事が公開されない
* slug は半角英数字とハイフンの組み合わせで、12 〜 50 文字の範囲内でないといけない
  * これは `zenn new:article --slug <SLUG>` を実行したときに出るので従わなければいけない気がするが、このコマンドを使わずに記事のファイル名をこのルールに従わないものにして投稿したらどうなるかは未検証
